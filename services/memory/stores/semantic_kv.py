from __future__ import annotations

from typing import Iterable

from services.memory.formation.schemas import MemoryWritePlan
from services.memory.stores.base import JsonlMockStore, plan_record


class SemanticKvMockStore(JsonlMockStore):
    def __init__(self, base_dir: str) -> None:
        super().__init__(base_dir, "memory_semantic_kv.jsonl")

    def write_plans(self, *, session_id: str, plans: Iterable[MemoryWritePlan]) -> int:
        return self.append_records(
            plan_record(session_id, plan, store="semantic_kv", role="canonical")
            for plan in plans
            if plan.status == "planned" and plan.canonical_store == "semantic_kv"
        )

